  ...
  return (
     <div className="flex justify-center items-center gap-2 flex-col">
      ...
      <div className="w-full px-4 md:px-8 mt-6 ">
              ...
              >
                {horrorGames.slice(0, 9).map((game) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-10 mx-5">
         ...
         {horrorGames.slice(9, 18).map((game) => (
          <div
            key={game.id}   
      ...
      <div className="flex gap-4 my-12">
        <button onClick={() => setPage((prev) => Math.max(prev - 1,1))} disabled = {page === 1}   className="px-6 py-2 bg-gray-800 text-white rounded-xl disabled:opacity-50">Previous</button>
        
        <span className="px-6 py-2 text-lg font-bold">page: {page}</span>

        <button onClick={() => setPage((prev) => Math.max(prev + 1, 1))}  className="px-6 py-2 bg-gray-800 text-white rounded-xl">Next</button>
      </div>
    </div>
  )
}
